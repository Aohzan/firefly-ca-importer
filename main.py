...
if __name__ == "__main__":
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        config.set(
            SETTINGS_SECTION, AUTO_DETECT_TRANSFERS_FIELD, AUTO_DETECT_TRANSFERS_DEFAULT
        )
        ...
        config.set(
            CREDIT_AGRICOLE_SECTION,
            IMPORT_ACCOUNT_ID_LIST_FIELD,
            IMPORT_ACCOUNT_ID_LIST_DEFAULT,
        )
        config.set(
            CREDIT_AGRICOLE_SECTION,
            GET_TRANSACTIONS_PERIOD_DAYS_FIELD,
            GET_TRANSACTIONS_PERIOD_DAYS_DEFAULT,
        )
        config.set(
            CREDIT_AGRICOLE_SECTION,
            MAX_TRANSACTIONS_PER_GET_FIELD,
            MAX_TRANSACTIONS_PER_GET_DEFAULT,
        )
        config.set(
            CREDIT_AGRICOLE_SECTION,
            TRANSFER_SOURCE_TRANSACTION_NAME_FIELD,
            TRANSFER_SOURCE_TRANSACTION_NAME_DEFAULT,
        )
        config.set(
            CREDIT_AGRICOLE_SECTION,
            TRANSFER_DESTINATION_TRANSACTION_NAME_FIELD,
            TRANSFER_DESTINATION_TRANSACTION_NAME_DEFAULT,
        )
        ...
        logger.log(
            "Config file '"
            + CONFIG_FILE
            + "' created ! Please fill it and start this script again."
        )
        logger.log(
            "Wiki page to help you : https://github.com/Royalphax/credit-agricole-importer/wiki/Config-file"
        )
        ...
        ca_cli.password = credit_agricole_section.get(
            BANK_PASSWORD_FIELD, BANK_PASSWORD_DEFAULT
        )
        ca_cli.enabled_accounts = credit_agricole_section.get(
            IMPORT_ACCOUNT_ID_LIST_FIELD, IMPORT_ACCOUNT_ID_LIST_DEFAULT
        )
        ca_cli.get_transactions_period = credit_agricole_section.get(
            GET_TRANSACTIONS_PERIOD_DAYS_FIELD, GET_TRANSACTIONS_PERIOD_DAYS_DEFAULT
        )
        ca_cli.max_transactions = credit_agricole_section.get(
            MAX_TRANSACTIONS_PER_GET_FIELD, MAX_TRANSACTIONS_PER_GET_DEFAULT
        )
        ca_cli.validate()
        ca_cli.init_session()
        ...
        f3_cli.token = firefly3_section.get(
            PERSONAL_TOKEN_FIELD, PERSONAL_TOKEN_DEFAULT
        )
        f3_cli.auto_detect_transfers = (
            settings_section.get(
                AUTO_DETECT_TRANSFERS_FIELD, AUTO_DETECT_TRANSFERS_DEFAULT
            )
            == "True"
        )
        f3_cli.transfer_source_transaction = (
            credit_agricole_section.get(
                TRANSFER_SOURCE_TRANSACTION_NAME_FIELD,
                TRANSFER_SOURCE_TRANSACTION_NAME_DEFAULT,
            )
            .strip()
        ...
        f3_cli.transfer_destination_transaction = (
            credit_agricole_section.get(
                TRANSFER_DESTINATION_TRANSACTION_NAME_FIELD,
                TRANSFER_DESTINATION_TRANSACTION_NAME_DEFAULT,
            )
        ...
        f3_cli.init_auto_assign_values(
        ...
        # Start main process
        logger.log(
            "Process started, debug=" + str(debug) + ", save_logs=" + str(save_logs)
        )

        # Get Firefly3 accounts numbers
        f3_accounts = f3_cli.get_accounts(account_type="asset")
        f3_accounts_number = [
            account.get("attributes").get("account_number") for account in f3_accounts
        ]
        ...
        for account in ca_cli.get_accounts():
            ...
            if account.numeroCompte not in f3_accounts_number:
                logger.log("  -> Accoung not found ... ", end="")
            else:
            ...
            for ca_transaction in ca_cli.get_transactions(account.numeroCompte):
                transactions.add_transaction(ca_transaction, account.numeroCompte)
                if not debug:
                    logger.log(".", end="")

            logger.log(" " + str(len(transactions)) + " new transaction(s)")

            if len(transactions) > 0:
                logger.log(
                    "  -> Pushing data to Firefly3 instance ",
                    end=("\n\r" if debug else ""),
                )
        ...
        if f3_cli.auto_detect_transfers:
            logger.log(
                "-> Pushing transfers to Firefly3 instance ",
                end=("\n\r" if debug else ""),
            )
