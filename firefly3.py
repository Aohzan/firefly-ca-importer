...
class Firefly3Client:
    def __init__(self, logger, debug):
        ...
        self.transfer_source_transaction = (
            TRANSFER_SOURCE_TRANSACTION_NAME_DEFAULT.strip().split(",")
        )
        self.transfer_destination_transaction = (
            TRANSFER_DESTINATION_TRANSACTION_NAME_DEFAULT.strip().split(",")
        )
        self.headers = None

    def _post(self, endpoint, payload):
        response = requests.post(
            "{}{}".format(self.hostname, endpoint), json=payload, headers=self.headers
        )
        if response.status_code != 200:
            self.logger.error(
                "Request to your Firefly3 instance failed. Please double check your personal token. Error : "
                + str(response.json())
            )
    ...
    def validate(self):
        ...
        if (
            ...
            or self.token == PERSONAL_TOKEN_DEFAULT
        ):
            self.logger.error(
                "Your firefly3 personal token isn't 980 characters long or isn't set."
            )
    ...
    def init_auto_assign_values(
        self,
        a_rename_transaction_section,
        aa_budget_section,
        aa_category_section,
        aa_account_section,
        aa_tags_section,
        ...
        for key in a_rename_transaction_section.keys():
            self.a_rename_transaction[key] = [
                e.strip() for e in a_rename_transaction_section.get(key, "").split(",")
            ]
        for key in aa_budget_section.keys():
            self.aa_budget[key] = [
                e.strip() for e in aa_budget_section.get(key, "").split(",")
            ]
        for key in aa_category_section.keys():
            self.aa_category[key] = [
                e.strip() for e in aa_category_section.get(key, "").split(",")
            ]
        for key in aa_account_section.keys():
            self.aa_account[key] = [
                e.strip() for e in aa_account_section.get(key, "").split(",")
            ]
        for key in aa_tags_section.keys():
            self.aa_tags[key] = [
                e.strip() for e in aa_tags_section.get(key, "").split(",")
            ]
...
class Firefly3Transactions:
    def __init__(self, f3_cli: Firefly3Client, account_id):
        self.f3_cli = f3_cli
    ...
    def find_existing_transaction(
        self, account: str, date: str, amount: float, description: str
    ) -> bool:
        date_formated = date[0:10]
        self.f3_cli.logger.log(
            "find existing transaction", debug=True, other_tag="[CA PAYLOAD]"
        )
        if account not in transactions_cache:
            transactions_cache[account] = {}
        if date_formated not in transactions_cache[account]:
            self.f3_cli.logger.log(
                f"get transactions for account {account} and day {date}",
                debug=True,
                other_tag="[CA PAYLOAD]",
            )
            page = 1
            transactions = []
            while True:
                page_transactions = self.f3_cli._get(
                    endpoint=_TRANSACTIONS_ENDPOINT,
                    params={
                        "page": page,
                        "start": date_formated,
                        "end": date_formated,
                ...
                transactions.extend(page_transactions["data"])
                if page < page_transactions["meta"]["pagination"]["total_pages"]:
                    page += 1
                else:
                    break
            transactions_cache[account][date_formated] = transactions

        for transaction in transactions_cache[account][date_formated]:
            for subtrans in transaction["attributes"]["transactions"]:
                if (
                    float(subtrans["amount"]) == amount
                    and subtrans["description"] == description
                ):
                    self.f3_cli.logger.log(
                        f"skip existing transaction: {date_formated} - {description} - {amount}",
                        debug=False,
                        other_tag="[CA PAYLOAD]",
                    )
                    return True
    ...
    def add_transaction(self, ca_payload, account):
        ...
        transaction["description"] = (
            renames[0] if len(renames) > 0 else transaction_name
        )

        date = time.mktime(
            time.strptime(ca_payload["dateOperation"], "%b %d, %Y, %H:%M:%S %p")
        )
        ...
        if self.find_existing_transaction(
            account=account,
        ...
        if self.f3_cli.auto_detect_transfers and is_in_list(
            ...
            if ca_payload["montant"] > 0:
                ...
                transaction["source_name"] = (
                    accounts[0] if len(accounts) > 0 else "Cash account"
                )
        ...
        self.f3_cli.logger.log(
            str(payload),
            debug=True,
            other_tag=("" if not is_transfer else "[TRANSFER]"),
        )
    ...
    def post_transfers(f3transactions_list, f3_cli):
        ...
        for f3_from_transactions in f3transactions_list:
            ...
            detected_transfers = (
                detected_transfers
                + len(f3_from_transactions)
                - len(f3_from_transactions.payloads)
            )
            ...
            for date_out in f3_from_transactions.transfer_out.keys():
                ...
                for transfer_out in f3_from_transactions.transfer_out[date_out]:
                    ...
                    for f3_to_transactions in f3transactions_list:
                        ...
                        if (
                            f3_to_transactions.account_id
                            == f3_from_transactions.account_id
                        ):
                            continue
                        ...
                        for date_in in f3_to_transactions.transfer_in.keys():
                            ...
                            if date_in == date_out:
                                ...
                                for transfer_in in f3_to_transactions.transfer_in[
                                    date_in
                                ]:
                                    # Get the amount
        ...
        if detected_transfers % 2 != 0 or len(payloads) * 2 != detected_transfers:
            f3_cli.logger.log_newline(
                "WARN: Wrong quantity of transfers detected ("
                + str(detected_transfers)
                + ") for "
                + str(len(payloads))
                + ' payload(s). You must double check your "transfer-source-transaction-name" and "transfer-destination-transaction-name" because some transfers hadn\'t been recognized.'
            )
